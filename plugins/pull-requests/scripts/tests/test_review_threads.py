...
def load_script() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "review_threads", HERE.parent / "review_threads.py"
    )
...
class ReviewThreadsTest(unittest.TestCase):
    def test_paginates_both_connection_levels_before_filtering(self) -> None:
        module = load_script()
        ...
        def graphql(query: str, variables: dict[str, Any]) -> dict[str, Any]:
            ...
            if "repository(owner:" in query:
                return fixture(
                    "threads-1.json"
                    if variables["threadsCursor"] is None
                    else "threads-2.json"
                )
            return fixture(
                "open-comments-2.json"
                if variables["threadId"] == "open"
                else "outdated-comments-2.json"
            )
        ...
        self.assertEqual(
            [comment["body"] for comment in result["open"][0]["comments"]],
            ["open-1", "open-2"],
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                {"threadId": "open", "commentsCursor": "open-comments-1"},
                {
                    ...
                    "name": "skills",
                    "number": 18,
                    "threadsCursor": "threads-1",
                },
                {"threadId": "outdated", "commentsCursor": "outdated-comments-1"},
            ],
        )

    def test_command_supports_installed_and_source_script_roots(self) -> None:
        command = (HERE.parent.parent / "commands" / "pr-comment-review.md").read_text()
        self.assertIn(
            "CLAUDE_PLUGIN_ROOT:-$(git rev-parse --show-toplevel)/plugins/pull-requests",
            command,
        )
