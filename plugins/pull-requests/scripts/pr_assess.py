...
def classify_comment(text: str) -> str:
    ...
    if "?" in text or lower.startswith(
        ("why ", "what ", "how ", "can you", "could you", "should we")
    ):
        return "actionable"
    suggestion_terms = [
        "consider",
...
def ensure_worktree(
    ...
    for worktree in parse_worktree_list(root):
        ...
        if resolved_path == target_path.resolve():
            return WorktreeResult(
                status="reused", path=path, head=head, note="matched deterministic path"
            )
        if (
            head
            and head == head_ref_oid
            and resolved_path.is_relative_to(target_base.resolve())
        ):
            return WorktreeResult(
                status="reused", path=path, head=head, note="matched PR head commit"
            )
    ...
    if (
        ...
        and head_repo_name_with_owner != repo_name_with_owner
        and head_repo_url
    ):
    ...
    try:
        run(
            ["git", "worktree", "add", "--detach", str(target_path), fetch_target],
            cwd=root,
        )
...
def fetch_graphql(repo_owner: str, repo_name: str, pr_number: int) -> dict[str, Any]:
    ...
    payload = graphql_request(
        query, {"owner": repo_owner, "name": repo_name, "number": pr_number}
    )
...
def paginate_pull_request_connections(
    ...
    paginate_review_threads(
        repo_owner, repo_name, pr_number, pull_request["reviewThreads"]
    )


def paginate_commits(
    repo_owner: str, repo_name: str, pr_number: int, connection: dict[str, Any]
) -> None:
    query = """
...
def paginate_review_threads(
	...
	"""
    paginate_connection(
        repo_owner, repo_name, pr_number, connection, query, "reviewThreads"
    )
...
def paginate_review_thread_comments(thread: dict[str, Any]) -> None:
    ...
    if page_info.get("hasNextPage"):
        thread["commentPaginationNote"] = (
            "additional review-thread comments omitted after first 100"
        )
...
def build_report(root: Path, pr_number: int) -> str:
    ...
    if open_items:
        for item in open_items[:20]:
            lines.append(
                format_comment_item(item, kind=item["kind"], source=item["source"])
            )
    ...
    if open_items:
        ...
        lines.append(
            f"- Address {blocking} blocking item(s) and {actionable} question(s)/actionable comment(s) before posting replies."
        )
    else:
    ...
    if worktree.status == "created":
        ...
        lines.append(
            "- Worktree path conflict needs manual resolution before code changes."
        )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Assess a pull request and summarize actionable feedback."
    )
    parser.add_argument(
        "pr_number",
        nargs="?",
        help="Optional PR number. Defaults to the current branch PR.",
    )
