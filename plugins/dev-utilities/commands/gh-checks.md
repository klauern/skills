...
5. **Apply Guarded Fixes**
   - Auto-run without prompt:
     ...
     - Lock-file sync commands (`npm install`, `poetry lock` (Poetry 2.x regenerates without upgrading), `cargo update`)
...
- **Secrets/permissions**: Detect `Resource not accessible by integration` or missing secret names and direct the user to update GitHub secrets instead of guessing values. See [Secrets/Permissions Workflow](../skills/ci-failure-analyzer/references/workflows.md#secretspermissions-workflow) for detailed remediation.
